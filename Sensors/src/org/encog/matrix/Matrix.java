...
//sjb - changed for Java ME support
//public class Matrix implements Cloneable, Serializable, EncogPersistedObject {
public class Matrix implements EncogPersistedObject {
    ...
    /**
     * Random number generator used for this matrix
     * sjb - added since Java ME doesn't support Math.random() method
     */
    Random generator;
	...
	public Matrix(final boolean[][] sourceMatrix) {
        ...
        //sjb - Init random number generator
	...
	public Persistor createPersistor() {
        ...
        //throw new UnsupportedOperationException("Persistence not supported in Java ME"); //not supported
            return null;
	}
	...
	public boolean equals(final Matrix matrix, final int precision) {
		...
		final double test = org.encog.util.Math.pow(10.0, precision);
		if (Double.isInfinite(test) || test > Long.MAX_VALUE) {
			throw new MatrixError("Precision of " + precision
					+ " decimal places is not supported.");
		}

		final int actualPrecision = (int) org.encog.util.Math.pow(Matrix.DEFAULT_PRECISION,
				precision);
	...
	public int fromPackedArray(final Double[] array, final int index) {
		...
		for (int r = 0; r < getRows(); r++) {
			for (int c = 0; c < getCols(); c++) {
				this.matrix[r][c] = array[i++].doubleValue();  //sjb - added double-value
			}
		}
	...
	public void ramdomize(final double min, final double max) {
		for (int r = 0; r < getRows(); r++) {
			for (int c = 0; c < getCols(); c++) {
				//this.matrix[r][c] = Math.random() * (max - min) + min;  //sjb - no Math.random() function in Java ME
                this.matrix[r][c] = generator.nextDouble() * (max - min) + min;
			}
		}
	}
	...
	public Double[] toPackedArray() {
		...
		for (int r = 0; r < getRows(); r++) {
			for (int c = 0; c < getCols(); c++) {
                ...
                result[index++] = new Double(this.matrix[r][c]);  //sjb - added Double wrapper
			}
		}
